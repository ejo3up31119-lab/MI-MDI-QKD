...
% =========================================================================
% --- MI MDI Protocol Calculation ---
% =========================================================================

% --- System Parameters (MI MDI) ---
L = linspace(0, 800, 10^5);          % Distance (Alice/Bob to Charlie) (km)
...
alpha = 10000;                       % Multiplexing factor
P_dc = 1 - (1 - P_dc)^alpha;         % Adjusted dark count probability
...
for n = [1 3 5 7]
    ...
    t_A = (2 * L) ./ C + (1 * L) ./ (C * P_tot);                     % Time for Alice
    ...
    % Error rates considering dark counts and multiplexing
    e_p_AC_dc = (e_p_AC .* P_tot + 0.5 * ((1 - P_tot) * P_dc)) ./ (P_tot + (1 - P_tot) * P_dc);
    e_b_AC_dc = (e_b_AC .* P_tot + 0.5 * ((1 - P_tot) * P_dc)) ./ (P_tot + (1 - P_tot) * P_dc);
    e_p_BC_dc = (e_p_BC .* P_tot + 0.5 * ((1 - P_tot) * P_dc)) ./ (P_tot + (1 - P_tot) * P_dc);
    e_b_BC_dc = (e_b_BC .* P_tot + 0.5 * ((1 - P_tot) * P_dc)) ./ (P_tot + (1 - P_tot) * P_dc);
    ...
    mux_raw_key_rate_final = 1 * ((P_tot + (1 - P_tot) .* P_dc) .* mux_raw_key_rate);
...
% =========================================================================
% --- MI Lo-Chau Protocol Calculation ---
% =========================================================================

% --- System Parameters (MI Lo-Chau) ---
...
eta_OS = 0.99;                       % Optical switch efficiency
P_s = eta_s * eta_d * exp(-L/L_att); % Photon event probability
P_dc = 10^(-8);                      % Dark count probability (Reset before alpha adjustment)
T1 = 10;                             % Relaxation time (s)
T2 = 500 * 10^(-3);                  % Coherence time (s)

% Inherits 'alpha' from the previous section
P_dc = 1 - (1 - P_dc)^alpha;         % Adjusted dark count probability
...
legend(h_lgd, labels, ...
       ...
       'FontSize', 12, ...
       'Location', 'southwest');     % Can be changed to 'best' if needed
